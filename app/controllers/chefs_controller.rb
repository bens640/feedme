class ChefsController < ApplicationController
  ...
  def show
    @chef_confirmations = Reservation.where(chef_id:current_chef.id).includes(:recipe)
    @chef_reservations = Reservation.where(chef_id:nil).includes(:recipe)
    @profile = @chef
  end
  def update
    @chef = Chef.
  ...
  def chef_params
    params.
    require(:chef).
    permit(:email, :password,:first_name, :last_name, :password_confirmation, :city, :state, :zip, :phone, :old_password,:id)
  end
end
